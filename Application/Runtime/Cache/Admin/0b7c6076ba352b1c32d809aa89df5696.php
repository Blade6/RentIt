  ...
  <body>
    ...
    <div class="container-fluid">
      <div class="row">
        ...
        <script type="text/javascript">
          switch("/RCW_MVC/index.php/Admin/Message/nospeak.html"){
	...
	<div class="table-reposive">
		<table class="table table-striped">
			...
			<tbody>
				<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><form action="<?php echo U('Message/nospeak');?>" method="post">
						<tr>
							<td><?php echo ($data["identity"]); ?><input type="hidden" value="<?php echo ($data["identity"]); ?>" name="id"></td>
							<td><?php echo ($data["username"]); ?></td>
							<?php if(!$data["shutup"]){ echo "<td><button class=\"btn btn-danger\" type=\"submit\" name=\"nospeak\">禁言</button></td>"; echo "<td><button class=\"btn btn-primary\" type=\"submit\" name=\"speak\" disabled=\"true\">解除禁言</button></td>"; } else{ echo "<td><button class=\"btn btn-danger\" type=\"submit\" name=\"nospeak\" disabled=\"true\">禁言</button></td>"; echo "<td><button class=\"btn btn-primary\" type=\"submit\" name=\"speak\">解除禁言</button></td>"; } ?>
						</tr>
