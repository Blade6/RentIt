  ...
  <body>
    ...
    <div class="container-fluid">
      <div class="row">
        ...
        <script type="text/javascript">
          switch("/RentIt/index.php/Admin/Car/superManage.html"){
	...
	<div class="table-reposive">
		<table class="table table-striped">
			...
			<tbody>
				<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><form action="<?php echo U('Car/superManage');?>" method="post" onsubmit="return sure()">
						<tr>
							<td>
								<?php echo ($data["license_no"]); ?>
								<input type="hidden" value="<?php echo ($data["license_no"]); ?>" name="license_no">
							</td>
							<td><?php echo ($data["rent_fare"]); ?></td>
							<td><?php echo ($data["value"]); ?></td>
							<td>
								<?php if(!$data["flag"]) echo "否"; else echo "是"; ?>
							</td>
							<td><?php echo ($data["num"]); ?></td>
							<td><button class="btn btn-success" type="submit" name="edit_2">修改</button></td>
						</tr>
